# Social Trend + Reference Video Style Intelligence — Implementation Handoff

> **Claude review (2026-09-01, same night):** Accepted as the design of
> record for the reference-style / trend standing items. Fact-checked: the
> repo snapshots are exact (both HEADs current to the hour), the §38
> capability matrix is accurate, and the sequencing discipline (manual
> references before any social APIs; blind baseline-vs-styled comparison
> as the go/no-go gate) matches this project's culture. The concept×style
> scoring correction (§5) is the key idea and fits the existing
> multi-concept pipeline directly. Gated behind acceptance run #1 — with
> one standing exception: MVP #1 (single-reference style extractor) is
> self-contained and can start whenever the owner supplies a reference
> video. Extraction-quality risk (hook/narrative grammar via VLM on
> third-party video) is real; the blind gate is the insurance and must be
> held strictly.
...
# 16. Reference-analysis pipeline
...
Use deterministic measurements for measurable structure.

Use multimodal reasoning for semantics.

---

# 17. Deterministic extraction responsibilities

Prefer deterministic or signal-processing methods for:
...
cuts per minute
hard-cut / dissolve indications where detectable
audio energy
beat/onset grid
BPM estimate
cut-to-beat distance
motion magnitude
scale/zoom evidence where measurable
caption bounding-box geometry where available
caption appearance/disappearance times
caption chunk duration
```

The exact libraries may reuse current FFmpeg infrastructure rather than adding large dependencies.

Do not add a dependency because it is fashionable. Measure first whether current FFmpeg/OpenCV/Python utilities are sufficient.

---

# 18. Semantic extraction responsibilities
...
tone
B-roll function
whether a shot is setup / reaction / evidence / transition
caption emphasis semantics
why a cut appears to be editorially motivated
whether the style is transferable to academic/research content
```

Do not ask a VLM to estimate precise shot durations or BPM when deterministic code can measure them.
...
11. Where should soft style constraints enter `planning.py`?
12. How do we prevent pacing goals from fighting word snapping and grounding?
13. Which existing B-roll selection logic can be conditioned by target ratio?
14. How should payoff-position targets affect concept-to-plan compilation without misleading chronology?
15. Which J/L choices can be generated deterministically from speech/source availability?

---

## Canonical representation

16. Can the MVP remain entirely on `edit-plan.v1`?
17. Which desired features force an `edit-plan.v2`?
18. Is it better to extend v1 compatibly or define v2 explicitly?
19. What migration/test surface would v2 require?
20. Can OpenTake readback preserve every proposed new canonical field?

---

## Evaluation

21. What is the fastest real-footage baseline-vs-style A/B harness?
22. Which current benchmark utilities can be reused?
23. How will blinded outputs avoid revealing which is style-conditioned?
24. What acceptance criterion is sufficient to proceed to Trend Scout?

---

## Trend providers — later

25. Which current provider actually offers the best semantic creator/post discovery?
26. Which provider gives historical velocity or enough snapshots to compute it ourselves?
27. Which gives sound/music identifiers and useful trend metadata?
28. What reference-media access is permitted?
29. Can analysis occur temporarily without long-term media storage?
30. Which terms restrict derivative feature storage?
...
shot-duration distribution
cuts/minute
basic B-roll/A-roll estimate or semantic role estimate
audio energy
beat grid/BPM where music permits
hook type
narrative beat sequence
payoff position
basic caption behavior if detectable
```
